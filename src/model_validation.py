...
class ImageClassificationValidation(ValidationAbstract):
    ...
    def cross_validation_stratified(self, train_folder_path, model_class):
        ...
        actual, predicted = [], []
        # loop
        for train_paths, test_paths in rskf.split(train_folder_path, model_class.get_classes_array(train_image_names_list)):
            ...
            #predicted_value = (model_class.predict(x_test))
            predicted.extend(model_class.predict(x_test))
            test_image_names_list = []
            test_image_names_list = [os.path.basename(image_path) for image_path in x_test]
            #actual_value = (model_class.get_classes_array(test_image_names_list))
    ...
    def plot_metrics(self, cvacc, cm, classes,
                          normalize=False,
    ...
    def print_summary(self, actual, predicted, available_classes):
        ...
        # Plot metrics
        plt.figure()
        self.plot_metrics(cross_validated_accuracy*100, conf_matrix, classes=available_classes,title='Confusion matrix')
        plt.show()
